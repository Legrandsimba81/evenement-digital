...
export default function AuthForm({ initialMode = "signin" }: AuthFormProps) {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    if (res?.error) {
      setError(
        res.error.includes("vérifier votre adresse email")
          ? res.error
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-2xl dark:border-gray-800 dark:bg-gray-950 lg:flex-row">
      {/* Partie gauche */}
      <div className="relative hidden lg:flex lg:flex-col lg:justify-between p-8 lg:w-[44%] lg:p-10 overflow-hidden">
        <div
          className="absolute inset-0 bg-cover bg-center"
          style={{ backgroundImage: "url('/images/auth-bg.jpg')" }}
        />
        <div className="absolute inset-0 bg-white/50 dark:bg-gray-950/70 backdrop-blur-[2px]" />

        <div className="relative z-10">
          <div className="mb-6 flex h-12 w-12 items-center justify-center rounded-2xl bg-primary-500/15 text-primary backdrop-blur-sm">
            <Sparkles size={22} />
          </div>
        ...
        <div className="relative z-10 mt-8 rounded-2xl border border-primary-500/20 bg-white/70 p-4 text-sm text-gray-600 shadow-sm backdrop-blur-sm dark:border-gray-700 dark:bg-gray-900/70 dark:text-gray-300">
