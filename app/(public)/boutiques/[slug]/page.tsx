...
export default async function BoutiquePage({ params }: { params: { slug: string } }) {
  ...
  const profileImages = Array.isArray(shop.profile?.images)
    ? shop.profile.images.filter((img: any) => typeof img === "string")
    : [];
      ...
      <div className="max-w-5xl mx-auto bg-white dark:bg-gray-900 rounded-2xl shadow-xl overflow-hidden">
        ...
        <div className="p-6 md:p-8">
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Colonne gauche : description, portfolio, tarifs, etc. */}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-1">
                    {tags.map((tag: string) => (
                      <span
                        key={tag}
                        className="bg-gray-100 dark:bg-gray-700 px-3 py-1 rounded-full text-xs text-gray-600 dark:text-gray-300"
                      >
                        #{tag}
                      </span>
                    ))}
